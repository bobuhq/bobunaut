...
begin
  ...
  select
    coalesce(
      bool_or(
        lower(bsi.provider) = 'telegram'
        and bsi.verified = true
      ),
      false
    ),
    coalesce(
      bool_or(
        lower(bsi.provider) = 'x'
        and bsi.verified = true
      ),
      false
    )
  into
    v_telegram_verified,
    v_x_verified
  from public.builder_social_identities as bsi
  where bsi.builder_id = v_builder_id;

  update public.builder_mining_sessions as bms
  set status = 'completed'
  where bms.builder_id = v_builder_id
    and bms.status = 'active'
    and bms.ends_at <= now();

  select count(*)::integer
  into v_mining_days
  from public.builder_mining_sessions as bms
  where bms.builder_id = v_builder_id
    and bms.status in ('completed', 'claimed')
    and bms.ends_at <= now()
    and bms.ends_at - bms.started_at >= interval '24 hours';
  ...
  if v_eligible then
    ...
    select bma.unlocked_at
    into v_unlocked_at
    from public.builder_mars_access as bma
    where bma.builder_id = v_builder_id;
  else
    insert into public.builder_mars_access (
      builder_id,
      unlocked,
