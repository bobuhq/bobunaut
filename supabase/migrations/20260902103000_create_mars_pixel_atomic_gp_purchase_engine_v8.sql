...
alter table public.mars_pixel_purchase_intents
  add column if not exists gp_ledger_id uuid
    references public.builder_reward_ledger(id)
    on delete restrict;

create unique index if not exists
  mars_pixel_purchase_intents_gp_ledger_uidx
on public.mars_pixel_purchase_intents (
  gp_ledger_id
)
where gp_ledger_id is not null;

alter table public.mars_pixel_purchase_intents
  add constraint mars_pixel_purchase_intents_committed_gp_ledger_check
  check (
    status <> 'committed'
    or gp_ledger_id is not null
  );
...
declare
  ...
  v_ledger_id uuid;
  v_existing_ledger public.builder_reward_ledger%rowtype;
