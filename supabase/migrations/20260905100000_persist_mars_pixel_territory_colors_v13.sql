...
drop function if exists public.commit_mars_pixel_gp_purchase_v1(
  uuid,
  uuid
);
...
begin
  ...
  v_requested_color_key :=
    nullif(
      upper(trim(coalesce(p_requested_color_key, ''))),
      ''
    );

  if v_requested_color_key = 'AUTO' then
    v_requested_color_key := null;
  end if;

  if v_requested_color_key is not null
     and v_requested_color_key not in (
       'RAINBOW_01',
       'RAINBOW_02',
       'RAINBOW_03',
       'RAINBOW_04',
    ...
    raise exception 'MARS_PIXEL_INVALID_COLOR_KEY'
      using errcode = '22023';
  end if;
  ...
  if v_requested_color_key is null then
    ...
    where option_row.color_key = v_requested_color_key
...
'Service-role-only atomic Mars Pixel GP purchase commit. Revalidates commercial state, GP pricing, purchase intent, reservation, protected zones, ownership, advertiser ownership, Builder balance and final territory color adjacency. Optional requested color is server-validated; null or AUTO uses server-authoritative balanced color assignment. Atomically writes GP debit, owned allocation with persisted color, allocation events, reservation conversion and purchase commitment. Pending Network GP is never spendable.';
...
'Backward-compatible service-role Mars Pixel GP purchase commit. Delegates to the V13 server-authoritative color-aware commit using AUTO color selection.';
...
'Public Mars Pixel owned allocations with server-persisted territory color.';
