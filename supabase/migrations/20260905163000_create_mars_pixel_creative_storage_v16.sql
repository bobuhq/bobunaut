insert into storage.buckets (
  id,
  name,
  public,
  file_size_limit,
  allowed_mime_types
)
values (
  'mars-pixel-creatives',
  'mars-pixel-creatives',
  true,
  5242880,
  array[
    'image/jpeg',
    'image/png',
    'image/webp'
  ]
)
on conflict (id) do update
set
  public = excluded.public,
  file_size_limit = excluded.file_size_limit,
  allowed_mime_types = excluded.allowed_mime_types;

drop policy if exists "mars_pixel_creatives_insert_own"
on storage.objects;

create policy "mars_pixel_creatives_insert_own"
on storage.objects
for insert
to authenticated
with check (
  bucket_id = 'mars-pixel-creatives'
  and (storage.foldername(name))[1] = auth.uid()::text
);

drop policy if exists "mars_pixel_creatives_update_own"
on storage.objects;

create policy "mars_pixel_creatives_update_own"
on storage.objects
for update
to authenticated
using (
  bucket_id = 'mars-pixel-creatives'
  and owner_id = auth.uid()::text
)
with check (
  bucket_id = 'mars-pixel-creatives'
  and (storage.foldername(name))[1] = auth.uid()::text
);

drop policy if exists "mars_pixel_creatives_delete_own"
on storage.objects;

create policy "mars_pixel_creatives_delete_own"
on storage.objects
for delete
to authenticated
using (
  bucket_id = 'mars-pixel-creatives'
  and owner_id = auth.uid()::text
);
