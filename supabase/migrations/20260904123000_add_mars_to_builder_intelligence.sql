...
declare
  ...
  v_mars_colony jsonb := '{}'::jsonb;
  v_mars_buildings jsonb := '[]'::jsonb;
  v_ares_hidden_mission jsonb := '{}'::jsonb;
  v_ares_research jsonb := '{}'::jsonb;
  v_ares_discovery_archive jsonb := '[]'::jsonb;
  v_ares_landmarks jsonb := '[]'::jsonb;
  v_mars jsonb := '{}'::jsonb;
...
begin
  ...
  /*
   * Mars Intelligence snapshot.
   *
   * Mars/Ares state is intentionally separate from the general
   * Missions snapshot. Each optional read is isolated so a Builder
   * without Mars access or an active Colony can still use BOBU AI.
   */
  begin
    select to_jsonb(colony)
    into v_mars_colony
    from public.get_my_mars_colony_v2() as colony
    limit 1;

    v_mars_colony := coalesce(v_mars_colony, '{}'::jsonb);
  exception
    when others then
      v_mars_colony := '{}'::jsonb;
  end;
  ...
  begin
    select coalesce(jsonb_agg(to_jsonb(building)), '[]'::jsonb)
    into v_mars_buildings
    from public.get_my_mars_colony_base() as building;

    v_mars_buildings := coalesce(v_mars_buildings, '[]'::jsonb);
  exception
    when others then
      v_mars_buildings := '[]'::jsonb;
  end;

  begin
    select to_jsonb(mission)
    into v_ares_hidden_mission
    from public.get_my_ares_hidden_mission() as mission
    limit 1;

    v_ares_hidden_mission :=
      coalesce(v_ares_hidden_mission, '{}'::jsonb);
  exception
    when others then
      v_ares_hidden_mission := '{}'::jsonb;
  end;
  ...
  begin
    ...
    into v_ares_research
    from public.builder_mars_research r
    join public.mars_hidden_mission_catalog c
      on c.mission_key = r.mission_key
    where r.builder_id = v_builder_id
      and r.sector_key = 'ares'
    order by r.cycle_key desc, r.created_at desc
    limit 1;

    v_ares_research := coalesce(v_ares_research, '{}'::jsonb);
  exception
    when others then
      v_ares_research := '{}'::jsonb;
  end;

  begin
    select coalesce(jsonb_agg(to_jsonb(discovery)), '[]'::jsonb)
    into v_ares_discovery_archive
    from public.get_my_ares_discovery_archive() as discovery;

    v_ares_discovery_archive :=
      coalesce(v_ares_discovery_archive, '[]'::jsonb);
  exception
    when others then
      v_ares_discovery_archive := '[]'::jsonb;
  end;

  begin
    select coalesce(jsonb_agg(to_jsonb(landmark)), '[]'::jsonb)
    into v_ares_landmarks
    from public.get_my_ares_landmark_discoveries() as landmark;

    v_ares_landmarks :=
      coalesce(v_ares_landmarks, '[]'::jsonb);
  exception
    when others then
      v_ares_landmarks := '[]'::jsonb;
  end;

  v_mars := jsonb_build_object(
    'available',
      v_mars_colony <> '{}'::jsonb
      or v_ares_hidden_mission <> '{}'::jsonb
      or v_ares_research <> '{}'::jsonb
      or jsonb_array_length(v_ares_discovery_archive) > 0
      or jsonb_array_length(v_ares_landmarks) > 0,
    'colony', v_mars_colony,
    'buildings', v_mars_buildings,
    'ares', jsonb_build_object(
      'hidden_mission', v_ares_hidden_mission,
      'research', v_ares_research,
      'discovery_archive', v_ares_discovery_archive,
      'landmarks', v_ares_landmarks
    )
  );
...
comment on function public.get_my_builder_intelligence() is
  'Returns a read-only Builder Intelligence snapshot including authenticated Mars and Ares state.';
