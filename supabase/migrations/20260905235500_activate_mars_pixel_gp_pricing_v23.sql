...
-- ============================================================
-- BOBU MARS PIXEL V23
-- Pixel-native GP settlement pricing activation
--
-- Commercial settlement:
--   1 Mars Pixel = 100 GP
--   Minimum purchase = 50 pixels
--
-- Reference valuation:
--   1 Mars Pixel = USD 1.00
--
-- NOTE:
-- price_per_block is retained for backward compatibility with
-- the existing purchase-intent / atomic purchase engine.
-- Since the Mars Pixel reservation model is now pixel-native
-- and block_count = pixel_count, this column represents the
-- settlement price PER PIXEL.
-- ============================================================


-- ------------------------------------------------------------
-- 1. Activate server-authoritative GP pricing
-- ------------------------------------------------------------
...
begin
  ...
  if not exists (
    ...
    raise exception 'MARS_PIXEL_PRICING_CONFIG_MISSING'
      using errcode = '55000';
  end if;
  ...
  set
    ...
    -- Pixel-native meaning from V23 onward: GP per Mars Pixel.
    price_per_block = 100,
    ...
    updated_at = now()
  where id = 1
    and grid_version = v_grid_version;
end;
$$;


comment on column public.mars_pixel_pricing_config.price_per_block is
'Legacy compatibility column. From Mars Pixel V23 onward, because block_count equals pixel_count in the pixel-native purchase path, this value is the GP settlement price per individual Mars Pixel.';


-- ------------------------------------------------------------
-- 2. Pixel-native quote engine
-- ------------------------------------------------------------
...
begin
  ...
  -- Reference valuation overflow protection.
  ...
  if v_reference_price is not null then
    ...
    v_reference_total := null;
  end if;

  -- Settlement overflow protection.
  if v_price_per_pixel is not null
     and v_pixel_count > 0
     and v_price_per_pixel >
       9223372036854775807::bigint /
       v_pixel_count::bigint then
  ...
  if v_price_per_pixel is not null then
    v_settlement_total :=
      v_price_per_pixel * v_pixel_count::bigint;
  else
  ...
  if v_pixel_count < v_minimum_pixels then
    ...
    v_quote_status := 'commercial_locked';
        ...
        or v_price_per_pixel is null then
    v_quote_status := 'settlement_unconfigured';

  elsif v_settlement_currency <> 'GP' then
    v_quote_status := 'unsupported_settlement_currency';
  ...
  v_quotable :=
    v_quote_status = 'quoted'
    and v_selection_status = 'available'
    and v_commercial_status = 'active'
    and v_pricing_status = 'configured'
    and v_settlement_currency = 'GP'
    and v_price_per_pixel is not null
    and v_settlement_total is not null;
...
'Server-authoritative pixel-native Mars Pixel quote engine. V23 enables GP settlement pricing per individual Mars Pixel while preserving USD reference valuation, minimum purchase size, commercial lock, protected-zone, ownership and active-reservation overlap checks. No GP debit, reservation creation, ownership mutation or checkout occurs in this read-only quote function.';


-- ------------------------------------------------------------
-- 3. Migration-time invariants
-- ------------------------------------------------------------
...
begin
  ...
  into
    ...
    v_reference_price,
    v_minimum
  from public.mars_pixel_pricing_config
  where id = 1
    and grid_version = v_grid_version;

  if v_status <> 'configured' then
    raise exception 'V23_ASSERT_PRICING_NOT_CONFIGURED';
  end if;

  if v_currency <> 'GP' then
    raise exception 'V23_ASSERT_SETTLEMENT_NOT_GP';
  end if;

  if v_price <> 100 then
    raise exception 'V23_ASSERT_GP_PRICE_NOT_100';
  end if;

  if v_reference_currency <> 'USD'
     or v_reference_price <> 100 then
    raise exception 'V23_ASSERT_REFERENCE_PRICE_INVALID';
  end if;

  if v_minimum <> 50 then
    raise exception 'V23_ASSERT_MINIMUM_NOT_50';
  end if;
end;
$$;
